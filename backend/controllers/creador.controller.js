...
exports.putPerfil = async (req, res) => {
  try {
    ...
    const perfil = await creadorService.actualizarPerfil(creadorId, descripcion, fotoUrl, bannerUrl);
...
exports.postMeta = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error al crear la meta.", error: error.message });
  }
};
...
exports.getReporteIngresos = async (req, res) => {
  try {
    ...
    const reporte = await creadorService.obtenerReporteFlanes(creadorId, fechaInicio, fechaFin);
...
exports.getListaCreadores = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error al listar creadores.", error: error.message });
  }
};
...
exports.getPerfilCreador = async (req, res) => {
  try {
    ...
    const perfilCompleto = await creadorService.obtenerPerfilPublico(creadorId, seguidorId);
